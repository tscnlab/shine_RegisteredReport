...
vidafar_001 <- vidafar_001 %>%
  mutate(reference = "Vidafar et al. (2024)",
         ...
         outcome_measured = "Sex difference in melatonin concentration at 400 lux",
...
vidafar_002 <- vidafar_002 %>%
   mutate(reference = "Vidafar et al. (2024)",
         ...
         outcome_measured = "Sex difference in melatonin concentration at 2000 lux",
...
The results are reported as means, however no SD or SEM is provided:
- 53% vs 13.8% at 3.00h,p<0.05 (women vs men),
- 64% vs 25.8% at 3.30h,p<0.05 (women vs men),
- 70.5% vs 26.3% at 4.00h,p<0.04 (women vs men)
From Figure 2, we can however manually extract the data point corresponding to each error bar using [WebPlotDigitizer](https://automeris.io/) and exporting the csv containing the measured data points. This then allows us to calculate an effect size.
...
## Note the the mean value for women at 03:30 does not add up to the one reported in the paper?

### Once we have the datapoints for each timepoint (03:00, 03:30, 04:00), how to deal with the extracted standard deviations?
```

#### Analysis code: monteleone_002

Area under the curve - Same problem as above?
...
monteleone_003 <- monteleone_003 %>%
  mutate(reference = "Monteleone et al. (1995)",
         ...
         outcome_measured = "Effect of gender on melatonin suppression",
...
This analysis used repeated measures ANOVA with main effect of sex to assess sex differences in melatonin levels at different light intensities (N=10, 5 women). The authors report the F statistic for sex differences in melatonin suppression, averaged across illuminance and time, F=0.003.
...
From this table, we extract information from the 1000 lux condition at the 90 minute timepoint, since this combination is the most relevant for the study we are designing and estimating the power for. Specifically, we will apply a light of approximately 400 lux as stimulus, hence the 1000 lux condition in the study by Boyce & Kennaway is the closest to ours in terms of illuminance levels.
...
boyce_kennaway_002 <- boyce_kennaway_002 %>%
   mutate(reference = "Boyce & Kennaway (1987)",
         ...
         outcome_measured = "Sex difference in melatonin concentration at 1000 lux",
...
nathan_002 <- nathan_002 %>%
  mutate(reference = "Nathan et al. (1997)",
         ...
         outcome_measured = "Sex difference in area under the curve of melatonin concentration at 200 lux",
...
nathan_003 <- nathan_003 %>%
  mutate(reference = "Nathan et al. (1997)",
         ...
         outcome_measured = "Sex differences in mean melatonin suppression (%) at 200 lux",
...
nathan_004 <- nathan_004 %>%
  mutate(reference = "Nathan et al. (1997)",
         ...
         outcome_measured = "Sex differences in area under the curve of melatonin concentration at 500 lux",
...
nathan_005 <- nathan_005 %>%
  mutate(reference = "Nathan et al. (1997)",
         ...
         outcome_measured = "Sex differences in mean melatonin suppression (%) at 500 lux",
...
```{r}
## The slope represents the rate of change in DV (melatonin suppression) for a unit increase in the independent variable (light intensity) -- reflects strength of relationship

## The intercept represents the value of the DV when IV is zero -- reflects baseline starting point (?) 
...
nathan_008 <- nathan_008 %>%
  mutate(reference = "Nathan et al. (1999)",
         ...
         outcome_measured = "Effect of menstrual cycle (luteal, follicular, menstrual) on melatonin suppression at 200 lux",
...
parry_001 <- parry_001 %>%
  mutate(reference = "Parry et al. (1997)",
       ...
       outcome_measured = "Differences in mean melatonin values at 500 lux during the follicular and luteal phase",
...
In this analysis, we calculate the effect size of the change score nights 1 to 3 (see column in Table 2) between the follicular and luteal phase. Change score of offset time: 
- 0.60±0.65 (military time, follicular phase),
- 0.70±0.91 (military time, luteal phase)
We take the same approach as parry_001 to calculate an effect size for this analysis, since we are facing the same type of data: repeated measures and within group design.
...
# print just the d value and confidence intervals
parry_002 <- data.frame(d = apa(stats_p002$d),
                        dlow = apa(stats_p002$dlow),
                        dhigh = apa(stats_p002$dhigh))
...
parry_002 <- parry_002 %>%
  mutate(reference = "Parry et al. (1997)",
       ...
       outcome_measured = "Change score in melatonin offset time between night 1 and night 3 of circadian phase shift study during the follicular and luteal phase",
...
From Table 2, we can also extract the change score (delta) in onset time of melatonin between night 1 and night 2 of the circadian phase shift study:
- 0.20±0.27 (military time, follicular phase),
- 0.40±0.65 (military time, luteal phase)
...
# print just the d value and confidence intervals
parry_003 <- data.frame(d = apa(stats_p003$d),
                        dlow = apa(stats_p003$dlow),
                        dhigh = apa(stats_p003$dhigh))
...
parry_003 <- parry_003 %>%
  mutate(reference = "Parry et al. (1997)",
       ...
       outcome_measured = "Change score in melatonin onset time between night 1 and night 3 of circadian phase shift study during the follicular and luteal phase",
       effsize_calculated = "d=-0.44, 95% CI [-1.34,0.51]")
```

#### Analysis code: parry_004

From Table 2, we extract the change score (delta) in duration (hours) of melatonin between night 1 and night 2 of the circadian phase shift study:
- -0.10±0.76 (hours, follicular phase),
- -0.30±1.03 (hours, luteal phase)
...
# print just the d value and confidence intervals
parry_004 <- data.frame(d = apa(stats_p004$d),
                        dlow = apa(stats_p004$dlow),
                        dhigh = apa(stats_p004$dhigh))
...
parry_004 <- parry_004 %>%
  mutate(reference = "Parry et al. (1997)",
       ...
       outcome_measured = "Change score in melatonin duration between night 1 and night 3 of circadian phase shift study during the follicular and luteal phase",
       effsize_calculated = "d=0.22, 95% CI [-0.68,1.01]")
```

#### Analysis code: parry_005

From Table 2, we extract the change score (delta) in melatonin peak (pg/ml) between night 1 and night 2 of the circadian phase shift study:
- -5.29±9.00 (follicular phase),
- -0.83±4.50 (luteal phase)
...
# print just the d value and confidence intervals
parry_005 <- data.frame(d = apa(stats_p005$d),
                        dlow = apa(stats_p005$dlow),
                        dhigh = apa(stats_p005$dhigh))
...
parry_005 <- parry_005 %>%
  mutate(reference = "Parry et al. (1997)",
       ...
       outcome_measured = "Change score in melatonin peak between night 1 and night 3 of circadian phase shift study during the follicular and luteal phase",
       effsize_calculated = "d=-0.66, 95% CI [-1.61,0.35]")
```

#### Analysis code: parry_006

From Table 2, we extract the change score (delta) in melatonin area under the curve (AUC, pg/ml) between night 1 and night 2 of the circadian phase shift study:
- -662±1495 (follicular phase),
- -2388±5138 (luteal phase)
...
# print just the d value and confidence intervals
parry_006 <- data.frame(d = apa(stats_p006$d),
                        dlow = apa(stats_p006$dlow),
                        dhigh = apa(stats_p006$dhigh))
...
parry_006 <- parry_006 %>%
  mutate(reference = "Parry et al. (1997)",
       ...
       outcome_measured = "Change score in melatonin AUC between night 1 and night 3 of circadian phase shift study during the follicular and luteal phase",
...
In this analysis, we look at the values in the column "Light exposure" for the NC group between the mid-follicular and late-luteal phase:
- 58.0±31.8 (mid-follicular phase),
- 62.7±39.6 (late-luteal phase)
...
# print just the d value and confidence intervals
parry_007 <- data.frame(d = apa(stats_p007$d),
                        dlow = apa(stats_p007$dlow),
                        dhigh = apa(stats_p007$dhigh))
...
parry_007 <- parry_007 %>%
  mutate(reference = "Parry et al. (2010)",
       ...
       outcome_measured = "Mean plasma melatonin levels during light exposure at 200 lux in mid-follicular and late-luteal phases",
...
parry_008 <- data.frame(reference = "Parry et al. (2010)",
       ...
       outcome_measured = "% melatonin suppression differences in response to 200 lux from baseline in the mid-follicular and late-luteal phases",
...
The authors measure reaction time following minimal melatonin suppression upon 6.5 hours light exposure during the follicular and luteal phases, N=29 (all women, N=12 follicular phase, N=17 luteal phase). They report Cohen's d directly in the paper, as an effect size for the difference in reaction times in women in follicular phase during minimal melatonin suppression.
```{r}
grant_001 <- data.frame(
  reference = "Grant et al. (2024)",
  doi = "10.1016/j.sleh.2023.08.012",
  analysis_code = "grant_001",
  outcome_measured = "Difference in reaction times for women in follicular vs luteal phase following minimal melatonin suppression upon light exposure",
  effsize_calculated = "d=1.9") 
```

#### Analysis code: grant_002
The authors measure attentional lapses following minimal melatonin suppression upon 6.5 hours light exposure during the follicular and luteal phases, N=29 (all women, N=12 follicular phase, N=17 luteal phase). They report Cohen's d directly in the paper, as an effect size for the difference in attentional lapses in women in follicular phase during minimal melatonin suppression.
```{r}
grant_002 <- data.frame(
  reference = "Grant et al. (2024)",
  doi = "10.1016/j.sleh.2023.08.012",
  analysis_code = "grant_002",
  outcome_measured = "Difference in attentional lapses for women in follicular vs luteal phase following minimal melatonin suppression upon light exposure",
  effsize_calculated = "d=1.7") 
```

#### Analysis code: grant_003
The authors measure reaction time following substantial vs minimal melatonin suppression upon 6.5 hours light exposure during the follicular phases, N=12 follicular phase. They report Cohen's d directly in the paper, as an effect size for the difference in reaction time in women in follicular phase when melatonin suppression was substantial vs minimal.
```{r}
grant_003 <- data.frame(
  reference = "Grant et al. (2024)",
  doi = "10.1016/j.sleh.2023.08.012",
  analysis_code = "grant_003",
  outcome_measured = "Difference in reaction time for women in follicular phase following substantial vs minimal melatonin suppression upon light exposure",
  effsize_calculated = "d=1.7") 
```

#### Analysis code: grant_004
The authors measure attentional lapses following substantial vs minimal melatonin suppression upon 6.5 hours light exposure during the follicular phases, N=12 follicular phase. They report Cohen's d directly in the paper, as an effect size for the difference in attentional lapses in women in follicular phase when melatonin suppression was substantial vs minimal.
```{r}
grant_004 <- data.frame(
  reference = "Grant et al. (2024)",
  doi = "10.1016/j.sleh.2023.08.012",
  analysis_code = "grant_004",
  outcome_measured = "Difference in attentional lapses for women in follicular phase following substantial vs minimal melatonin suppression upon light exposure",
...
fazlali_001 <- data.frame(
  reference = "Fazlali et al. (2024), BioRxiv",
  doi = "10.1101/2024.10.18.619012",
  analysis_code = "fazlali_001",
  outcome_measured = "Effect of menstrual phase (follicular and menstrual) on melatonin AUC",
...
fazlali_002 <- data.frame(
  ...
  outcome_measured = "Effect of sex on melatonin AUC",
  effsize_calculated = "ωp2=0") 
```
...
```{r}
# Combining all data frames into one to create a gt table

## First, create a list 
df_list <- list(vidafar_001,
                 vidafar_002,
                 monteleone_003,
                 boyce_kennaway_002,
                 nathan_001,
...
# Turn effisze_calculated column to character for all dfs
df_list <- lapply(df_list, function(df) { # we use lapply to apply the function to each df in the list, and return a list of the same length 
  df %>% mutate(effsize_calculated = as.character(effsize_calculated))
})

# Combine all data frames into one
effsizes_df_combined <- bind_rows(df_list) %>%
