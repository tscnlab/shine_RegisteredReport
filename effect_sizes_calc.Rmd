...
Change approach below! Only one - no need for a b c

```{r}
# Need to calculate effect size for each timepoint, starting at 03:00

## 03:00
stats_mont001_a <- MOTE::d.ind.t(
  m1 = mont_fig02$f_light_sens[1],
  m2 = mont_fig02$m_light_sens[1],
  sd1 = mont_fig02$f_sd_diff[1],
  sd2 = mont_fig02$m_sd_diff[1],
  n1 = 6,
  n2 = 6,
  a = 0.05
)
...
## 03:30
stats_mont001_b <- MOTE::d.ind.t(
  m1 = mont_fig02$f_light_sens[2],
  m2 = mont_fig02$m_light_sens[2],
  sd1 = mont_fig02$f_sd_diff[2],
  sd2 = mont_fig02$m_sd_diff[2],
  n1 = 6,
  n2 = 6,
  a = 0.05
)
...
## 04:00
stats_mont001_c <- MOTE::d.ind.t(
  m1 = mont_fig02$f_light_sens[3],
  m2 = mont_fig02$m_light_sens[3],
  sd1 = mont_fig02$f_sd_diff[3],
  sd2 = mont_fig02$m_sd_diff[3],
  n1 = 6,
  n2 = 6,
  a = 0.05
)
...
monteleone_001 <- data.frame(
  d_a = apa(stats_mont001_a$d), 
  dlow_a = apa(stats_mont001_a$dlow), 
  dhigh_a = apa(stats_mont001_a$dhigh),
  d_b = apa(stats_mont001_b$d),
  dlow_b = apa(stats_mont001_b$dlow),
  dhigh_b = apa(stats_mont001_b$dhigh),
  d_c = apa(stats_mont001_c$d),
  dlow_c = apa(stats_mont001_c$dlow),
  dhigh_c = apa(stats_mont001_c$dhigh)
  )

# Switch to long format
monteleone_001_long <- monteleone_001 %>%
  pivot_longer(
    cols = everything(),  # Select all columns to pivot
    names_to = c(".value", "group"),  # ".value" keeps d, dlow, dhigh as separate columns
    names_pattern = "(d[a-z]*)_([a-c])"  # Correctly separates "d", "dlow", "dhigh" from "a", "b", "c"
  )

# Take average of these three timepoints
monteleone_001_long <- monteleone_001_long %>%
  summarise(
    d = mean(as.numeric(d), na.rm = TRUE),
    dlow = mean(as.numeric(dlow), na.rm = TRUE),
    dhigh = mean(as.numeric(dhigh), na.rm = TRUE)
  )
...
monteleone_001 <- monteleone_001 %>%
  mutate(reference = "Monteleone et al. (1995",
         ...
         outcome_measured = "Sex difference in melatonin concentration at 200 lux",
         effsize_calculated = "d=0.71, 95% CI [0.16, 1.25]")
```
...
nathan_006 <- data.frame(
  d = apa(stats_nat006$d), 
  dlow = apa(stats_nat006$dlow), 
  dhigh = apa(stats_nat006$dhigh))
...
```{r}
# Combining all data frames to create two gt tables (one would be too long)

## First, create a list for table 1
df_list_1 <- list(vidafar_001,
                 vidafar_002,
                 monteleone_003,
                 boyce_kennaway_002,
                 nathan_001,
...
effsizes_df1_combined <- bind_rows(df_list_1) %>%
  ...
  dplyr::mutate(a = "")  # Add an empty column - workaround because otherwise the last col will be truncated in the gt table

## Second, create a list for table 2
df_list_2 <- list(parry_003,
                 parry_004,
                 parry_005,
                 parry_006,
                 parry_007,
...
effsizes_table_1 <- effsizes_df1_combined %>%
  gt::gt() %>%
  gt::tab_header(title = gt::md("**Calculation of effect sizes for studies on sex- and hormone-related differences in non visual responses to light exposure**")) %>%
  ...
  gt::tab_options(
    table.font.size = gt::px(14)  # Adjust as needed
  )
...
effsizes_table_2 <- effsizes_df2_combined %>%
  ...
  gt::cols_label(reference = "Reference",
                 ...
                 effsize_calculated = "Calculated effect size") %>%
  gt::tab_options(
    table.font.size = gt::px(16)  # Adjust as needed
  )
```
...
gt::gtsave(effsizes_table_1,
           filename = "effsize_table_pt1.png",
...
gt::gtsave(effsizes_table_2,
           filename = "effsize_table_pt2.png",
           path = here::here("outputs"),
           vwidth = 1400,
           vheight = 300)
