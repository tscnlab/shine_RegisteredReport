...
```
This shows that more than 50% of the simulated data does not converge. 

## Reduced model 1
We remove random slopes for one of the hormones
```{r}
model_reduced_1 <- y ~ e2_value + p4_value + (1 + p4_value | id)

# Run diagnostics for model_reduced_1
diag_model_reduced_1 <- run_model_diagnostics(model_maximal, all_sim_data)

# Turning the model diagnostics into a dataframe
diagnostic_overview_model_reduced_1 <- do.call(rbind, lapply(diagnostic_overview_model_reduced_1, function(x) {
  if (!is.list(x) || !"name" %in% names(x)) return(NULL)
  data.frame(
    sim_name = x$name,
    is_singular = ifelse("is_singular" %in% names(x), x$is_singular, NA),
